  ...
  const spec = useMemo(
    () => [
      vg.ruleX(vg.from(TABLES.planckDl.name), {
        ...
        stroke: chartPalette.errorStroke, strokeWidth: 1.2, strokeOpacity: 0.7,
      }),
      vg.dot(vg.from(TABLES.planckDl.name), {
        x: "ell", y: "Dl", r: 3,
        fill: chartPalette.dataFill, stroke: chartPalette.dataStroke, strokeWidth: 1,
      }),
      vg.line(modelCurve.map((row) => ({ ell: row.ell, Dl: row.Dl })), {
        x: "ell", y: "Dl", stroke: chartPalette.modelStroke, strokeWidth: 2,
      }),
      vg.xLabel("Multipole ℓ →"),
      vg.yLabel("↑ Dℓ = ℓ(ℓ+1)Cℓ/2π  (μK²)"),
      vg.xDomain([0, 2550]),
      vg.yDomain([0, 7000]),
      vg.width(820), vg.height(480),
  ...
  return (
    <PlotSection
      ...
      math={<>
        ...
        <Text fontSize="sm" color="navy.200"><MathInline>{`C_\\ell`}</MathInline> is the variance of the ℓ-th multipole of the temperature map; multiplying by <MathInline>{`\\ell(\\ell+1)/2\\pi`}</MathInline> gives a curve whose features sit at roughly the same <MathInline>{`D_\\ell`}</MathInline> across many decades of ℓ. The first peak position <MathInline>{`\\ell_1 \\approx 220`}</MathInline> sets the angular size of the sound horizon at last scattering and therefore the geometry of the universe; the peak ratios fix the baryon density.</Text>
      </>}
      plot={error !== null ? <PlotError message={error} /> : <MosaicPlot spec={spec} enabled={ready} ariaLabel="CMB angular power spectrum: binned Dℓ data with error bars and ΛCDM theory line" minHeight="480px" />}
      controls={
      ...
      citation={<Citation title="Data source & provenance"><Text>Simulated Planck-like binned TT spectrum (45 bins, ℓ ∈ [30, 2500]) generated by <Code>scripts/simulate/cmb_powerspectrum.ts</Code> from the analytical model in <Code>src/physics/cmb.ts</Code>. Real source: Planck Collab. (2020), A&A 641, A6; doi:10.1051/0004-6361/201833910. The fiducial cosmology used for generation is Planck 2018 base-ΛCDM.</Text><Text mt={2}>See <Link href="https://github.com/karthikbadam/known-universe/blob/main/scripts/fetch/cmb_powerspectrum.md" target="_blank" rel="noopener noreferrer">/scripts/fetch/cmb_powerspectrum.md</Link> for instructions to swap in the real Planck Legacy Archive file. The theory curve currently comes from a parameterized analytical model (~5-15% accurate vs CAMB); upgrade instructions are in the same fetch.md.</Text></Citation>}
    />
  );
}
