...
export function RotationCurves(): JSX.Element {
  const { ready, error } = useDataTable(
    TABLES.sparcGalaxies.name,
    TABLES.sparcGalaxies.url,
    { skipHeaderLines: TABLES.sparcGalaxies.skipHeaderLines },
  );
  ...
  return (
    <PlotSection
      ...
      summary={
        <Text>
          Newtonian gravity predicts that, beyond the visible disk, the orbital
          speed of stars and gas should fall as 1/√r. SPARC data says it doesn't
          — the curves stay flat or even rise. Either gravity breaks at
          galactic scales, or an unseen halo of dark matter contributes extra
          enclosed mass. The "baryonic" curve shows what visible mass alone
          predicts; the gap is what dark matter has to fill.
        </Text>
      }
      math={
        <>
          ...
          <Text fontSize="sm" color="navy.200">
            <MathInline>{`x = r/r_s`}</MathInline>. The total enclosed mass is
            the baryonic disk plus an NFW halo. The two sliders pick the halo
            scale radius and characteristic density; the total curve is the
            quadrature sum of baryonic and DM contributions.
          </Text>
        </>
      }
      plot={
        error !== null ? (
          ...
          <MosaicPlot
            spec={spec}
            enabled={ready}
            ariaLabel={`Rotation curve of ${galaxy} with model decomposition`}
            minHeight="440px"
          />
        )
      }
      ...
      citation={
        <Citation title="Data source & provenance">
          <Text>
            ...
            <Code>scripts/simulate/sparc.ts</Code> from the NFW + baryonic disk
            model in <Code>src/physics/nfw.ts</Code>. Real source: SPARC
            database, Lelli et al. (2016).{" "}
            <Link
