  ...
  useEffect(() => {
    ...
    for (let py = 0; py < CANVAS_H; py++) {
      ...
      for (let px = 0; px < CANVAS_W; px++) {
        ...
        const fx = Math.min(FIELD_W - 1, Math.max(0, Math.floor(((ll.lon + Math.PI) / (2 * Math.PI)) * FIELD_W)));
        const fy = Math.min(FIELD_H - 1, Math.max(0, Math.floor(((ll.lat + Math.PI / 2) / Math.PI) * FIELD_H)));
  ...
  return (
    <PlotSection
      ...
      math={<>
        ...
        <Text fontSize="sm" color="navy.200"><MathInline>{`Y_{\\ell m}`}</MathInline> are spherical harmonics; the variance of the <MathInline>{`a_{\\ell m}`}</MathInline> coefficients is the angular power spectrum <MathInline>{`C_\\ell`}</MathInline> plotted in the next section.</Text>
      </>}
      plot={
      ...
      controls={
        <SimpleGrid columns={{ base: 1, md: 1 }} gap={5}>
          <ParamSlider label="Temperature scale (± μK)" unit="μK" description="Maps the colormap range. Planck SMICA uses ±300 μK by convention." min={100} max={600} step={10} value={scale} onChange={setScale} />
        </SimpleGrid>
      }
      ...
      citation={<Citation title="Data source & provenance"><Text>Procedurally generated CMB-like field via <Code>src/physics/cmbMap.ts</Code> — 12 large Gaussian blobs on the sphere plus smoothed Gaussian speckle. Real source: Planck SMICA map (Planck 2018 results IV, A&A 641, A4). <Link href="https://github.com/karthikbadam/known-universe/blob/main/scripts/fetch/cmb_map.md" target="_blank" rel="noopener noreferrer">fetch.md</Link> has the curl-able all-sky PNG URL.</Text></Citation>}
    />
  );
}
