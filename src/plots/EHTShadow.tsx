...
export function EHTShadow() {
  ...
  return (
    <PlotSection
      ...
      plot={
        <Box display="flex" justifyContent="center" alignItems="center" h={`${PLOT_HEIGHT}px`}>
          ...
          >
            ...
            />
            <circle cx={0} cy={0} r={radiusInView} fill="none" stroke={targetStroke} strokeWidth={0.4} strokeOpacity={0.5} />
            <circle cx={0} cy={0} r={M87_OBSERVED.shadowDiameterUas / 2} fill="none" stroke={targetStroke} strokeWidth={0.6} strokeDasharray="2,1.5" />
            <line x1={-35} y1={32} x2={-15} y2={32} stroke={scaleColor} strokeWidth={0.6} />
            <text x={-25} y={36} fill={scaleColor} fontSize={3.2} textAnchor="middle">20 μas</text>
          </svg>
        </Box>
      }
      controls={
        <SimpleGrid columns={{ base: 1, md: 3 }} gap={5}>
          ...
          <Stat.Root bg="bg.subtle" p={3} borderRadius="sm" borderWidth={1} borderColor="border">
            <Stat.Label color="fg.muted" fontFamily="mono" fontSize="xs">Predicted shadow diameter</Stat.Label>
            <Stat.ValueText color={matches ? "accent" : "fg"} fontFamily="mono">{predictedDiameterUas.toFixed(1)} μas</Stat.ValueText>
            <Text color="fg.muted" fontSize="xs" mt={1}>EHT measured: 42 ± 3 μas</Text>
          </Stat.Root>
        </SimpleGrid>
      }
      ...
      citation={<Citation title="Data source & provenance"><Text>Real EHT M87* April 2019 image from ESO (<Code>eso1907a.jpg</Code>), overlaid with the dashed circle showing the predicted Schwarzschild shadow at the current slider values. Real source: EHT Collaboration (2019) ApJ 875, L1. <Link href="https://github.com/karthikbadam/known-universe/blob/main/scripts/fetch/eht_m87.md" target="_blank" rel="noopener noreferrer">/scripts/fetch/eht_m87.md</Link>.</Text><Text mt={2}>Shadow physics in <Code>src/physics/blackHoleShadow.ts</Code>: Schwarzschild b_crit, with M87 mass and distance from the EHT paper.</Text></Citation>}
    />
  );
}
