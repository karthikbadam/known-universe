import { Box, Code, Link, SimpleGrid, Stat, StatLabel, StatNumber, Text } from "@chakra-ui/react";
...
export function EHTShadow(): JSX.Element {
    ...
    <PlotSection
      ...
      summary={
        <Text>
          ...
          supermassive black hole. The dark disk's apparent diameter on the
          sky encodes only two numbers: the BH mass and its distance from
          us. Drag the mass slider and watch the predicted shadow grow; aim
          for the EHT measurement (42 ± 3 μas) and read off ~6.5 × 10⁹ M_☉.
        </Text>
      }
      math={
        <>
          <MathBlock ariaLabel="shadow diameter formula">
            {`\\theta_{\\rm sh} \\;=\\; \\frac{2 b_{\\rm crit}}{D} \\;=\\; \\frac{6\\sqrt{3}\\, G M}{c^2 D}`}
          </MathBlock>
          <Text fontSize="sm" color="navy.200">
            <MathInline>{`b_{\\rm crit} = 3\\sqrt{3}\\, G M / c^2`}</MathInline> is
            the critical impact parameter — light with smaller b is captured.
            The Schwarzschild radius{" "}
            <MathInline>{`R_s = 2GM/c^2`}</MathInline> is buried in there;
            the shadow is{" "}
            <MathInline>{`5.196 R_s`}</MathInline> across. For Kerr, spin
            shifts this by a few percent at modest inclinations.
          </Text>
        </>
      }
      plot={
        <Box display="flex" justifyContent="center" alignItems="center" py={4}>
          ...
          >
            ...
            <line
              ...
              strokeWidth={0.6}
            />
            <text x={-25} y={36} fill="#e9eef7" fontSize={3.2} textAnchor="middle">
              20 μas
            </text>
          </Box>
        </Box>
      ...
      controls={
        <SimpleGrid columns={{ base: 1, md: 3 }} spacing={5}>
          ...
          >
            <StatLabel color="navy.300" fontSize="xs">
              Predicted shadow diameter
            </StatLabel>
            <StatNumber color={matches ? "green.300" : "gold.300"}>
              {predictedDiameterUas.toFixed(1)} μas
            </StatNumber>
            <Text color="navy.300" fontSize="xs" mt={1}>
      ...
      citation={
        <Citation title="Data source & provenance">
          <Text>
            Image placeholder is an SVG render (warm photon ring around a
            central shadow). Real source: EHT Collaboration (2019) ApJ
            875, L1; image available from ESO and EventHorizonTelescope.org.{" "}
            <Link
            ...
            has the curl-able URLs.
          </Text>
          <Text mt={2}>
            Shadow physics in <Code>src/physics/blackHoleShadow.ts</Code> is
            real: Schwarzschild b_crit, M87 mass and distance from the EHT
            paper.
          </Text>
        </Citation>
      }
