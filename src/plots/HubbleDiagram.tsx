...
export function HubbleDiagram(): JSX.Element {
  ...
  // H0 default = 70 km/s/Mpc (modern); slider range spans Hubble's 1929 fit
  // (~500) so the user can drag the line to overlay the historical cloud.
  ...
  // Load the CSV + the model-grid table exactly once.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        // 45 points evenly spaced over [0, MAX_DISTANCE_MPC]. The model line
        // is y = H0 * d, computed in SQL with the Param injected.
        await coord.exec(`
  ...
  // Build the vgplot spec once data is loaded. H0 is bound through the
  // sql template, so the line re-renders whenever h0.update(...) fires
      ...
      citation={
        <Citation title="Data source & provenance">
          ...
          <Text mt={2}>
            See{" "}
            <Link
              href="https://github.com/karthikbadam/known-universe/blob/feat/cosmology-vis/scripts/fetch/hubble1929.md"
              isExternal
            >
