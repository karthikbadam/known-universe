...
export function GW150914() {
  ...
  const spec = useMemo(() => [
    vg.line(vg.from(TABLES.gw150914.name), { x: "t_s", y: "strain", stroke: palette.errorStroke, strokeWidth: 0.6, strokeOpacity: 0.7 }),
    vg.line(modelLine, { x: "t", y: "h", stroke: palette.dataFill, strokeWidth: 2 }),
  ...
  return (
    <PlotSection
      ...
      math={<>
        ...
        <Text fontFamily="body" fontSize="sm" color="fg.muted" lineHeight="1.7"><MathInline>{`M_c = (m_1 m_2)^{3/5}/(m_1+m_2)^{1/5}`}</MathInline> is the "chirp mass", the only combination of the binary masses the inspiral waveform depends on at leading PN order. Fitting it measures the source mass without ever resolving the two components.</Text>
      </>}
      plot={error !== null ? <PlotError message={error} /> : <MosaicPlot spec={spec} enabled={ready} ariaLabel="GW150914 strain time series with chirp model overlay" height={PLOT_HEIGHT} />}
      controls={
      ...
      citation={<Citation title="Data source & provenance"><Text>LIGO H1 strain around GW150914 from GWOSC (<Code>H-H1_LOSC_4_V2-1126259446-32.hdf5</Code>), whitened against the Welch PSD, Butterworth-bandpassed 35–350 Hz, decimated 4096 → 512 Hz, then rescaled so the noise standard deviation matches the simulated convention. Real source: Abbott et al. (LIGO 2016) PRL 116, 061102. See <Link href="https://github.com/karthikbadam/known-universe/blob/main/scripts/fetch/gw150914.md" target="_blank" rel="noopener noreferrer">/scripts/fetch/gw150914.md</Link>.</Text></Citation>}
    />
  );
}
