...
export function BAOFeature() {
  ...
  const spec = useMemo(() => [
    vg.ruleX(vg.from(TABLES.bossXi.name), { x: "s_mpc", y1: "xi_lower", y2: "xi_upper", stroke: chartPalette.errorStroke, strokeWidth: 1.2, strokeOpacity: 0.7 }),
    vg.dot(vg.from(TABLES.bossXi.name), { x: "s_mpc", y: "xi", r: 4, fill: chartPalette.dataFill, stroke: chartPalette.dataStroke }),
    vg.line(modelCurve.map((row) => ({ s: row.s, xi: row.xi })), { x: "s", y: "xi", stroke: chartPalette.modelStroke, strokeWidth: 2 }),
    vg.ruleX([{ x: rd }], { x: "x", stroke: chartPalette.highlightStroke, strokeWidth: 1.2, strokeDasharray: "4,3", strokeOpacity: 0.7 }),
    ...
    vg.width(820), vg.height(440), vg.marginLeft(75), vg.marginBottom(50),
  ], [modelCurve, rd]);
  ...
  return (
    <PlotSection
      ...
      math={<>
        ...
        <Text fontSize="sm" color="navy.200"><MathInline>{`c_s`}</MathInline> is the photon-baryon plasma sound speed; <MathInline>{`H(z)`}</MathInline> the Hubble rate. The integral gives the sound horizon at the drag epoch (z ≈ 1060). At late times it appears as the dashed vertical guide moving with the slider.</Text>
      </>}
      plot={error !== null ? <PlotError message={error} /> : <MosaicPlot spec={spec} enabled={ready} ariaLabel="BAO correlation function with a bump near 150 Mpc" minHeight="440px" />}
      controls={
