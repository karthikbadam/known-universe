...
```sh
# After signing in to https://pla.esac.esa.int/ and clicking through the
# data-use agreement:
...
## Option B — community mirrors (no registration)

Several CMB analysis libraries ship the same binned values, e.g. the
[cosmojs](https://github.com/cobaya/cobaya) and CLASS distributions
include the file. URLs change occasionally; check before relying.

```sh
# Example (verify path before use):
curl -fL -o public/data/planck_dl.csv \
  https://raw.githubusercontent.com/cobaya/cobaya/master/cobaya/likelihoods/planck_2018_lowl_TT/data/planck_2018_lowl_TT.dat
# Then renormalize and re-bin to ℓ ∈ [30, 2500] in 45 bins to match
